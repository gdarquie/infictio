...
if (isset ($_POST["modifier"]))
	{
		...
		$req = $bdd->prepare('SELECT * FROM disrepair WHERE id = ?');
...
<div class='container'>
	<section class='row'>
		...
		<article>
			<form method='post' action='index.php'>
				...
				<div id='langue'>
					...
					<select name='langue'>
						<option value='fr' <?php if(isset ($_POST["modifier"]) AND $langue == "fr" ) {echo "selected";} ?> >Français</option>
						<option value='en' <?php if(isset ($_POST["modifier"]) AND $langue == "en" ) {echo "selected";} ?> >English</option>
					</select>
				</div>
	                ...
	                if (isset ($_POST["modifier"]))
	                	{
	               		...
	               		{
	                		$req = $bdd->prepare('DELETE FROM disrepair WHERE id = ?');
		...
		<?php
		$i=0;
		$reponse = $bdd->query('SELECT * FROM disrepair ORDER BY id DESC');
